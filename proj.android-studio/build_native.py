...
lib_path = os.path.abspath(os.path.join('..', 'console'))
sys.path.append(lib_path)
import strings_list, resources_list
...
def build(build_mode):
    ...
    command = 'cocos compile -p android --android-studio --no-apk -s %s -m %s' % (app_android_root, build_mode) 
    if os.system(command) != 0:
        raise Exception("Build dynamic library for project [ " + app_android_root + " ] fails!")

# -------------- main --------------
if __name__ == '__main__':
    resources_list.write_result()
    strings_list.write_result()
